...
| Artifact 存储 | `zcli/artifacts.py` | Session 私有的大结果持久化、检查、搜索和分块读取 | `ArtifactStore`, `ArtifactMetadata` |
| 上下文管理 | `zcli/context.py` | 分层压缩、工具结果预算与 transcript 持久化 | `ContextManager` |
...
| 工具注册 | `zcli/tools.py` | 30 个内置工具与 MCP 动态工具的定义、权限和分发 | `ToolRegistry` |
