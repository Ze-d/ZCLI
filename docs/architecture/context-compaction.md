...
## 第一层：大工具结果转为 Session Artifact

单个工具结果超过 `persist_threshold`（默认 30,000 字符），或当前轮工具结果总量超过 `tool_result_budget_bytes`（默认 200,000 字符）时，完整输出写入：

```text
.zcli/artifacts/<session_id>/<artifact_id>/
  content.txt
  metadata.json
```

消息中的正文替换为 `<artifact-result>` 引用、大小信息以及 head/tail 预览。模型不再承担完整日志的 token 成本，可按需调用：

- `inspect_artifact`：查看元数据和首尾预览；
- `search_artifact`：按文本或正则搜索，返回行号、字符偏移和上下文；
- `read_artifact_chunk`：通过字符 offset 分块读取，单次最多 20,000 字符。

Artifact 只能在创建它的 Session 中读取。模型只提交 `artifact_id`，当前 `session.id` 由工具执行层隐式注入，因此 Artifact ID 只是资源标识，不是跨 Session 的访问凭证。

`bash` 和 `read_file` 不再在 Artifact 处理前截断结果，避免只保存前 50,000 字符。第一版仍会在内存中短暂持有完整文本，流式写盘属于后续优化。
...
```text
.zcli/
  artifacts/      # 按 Session 隔离的大型工具输出和元数据
  tool-results/   # 旧版大型工具输出，保留兼容
...
- 便宜层必须先于 LLM 摘要执行；
- 大型输出落盘后可以完整恢复；
- Artifact 不能被其他 Session 读取；
- Artifact 工具返回量保持在上下文安全预算内；
- 最近工具结果保持完整，旧结果才被微压缩；
