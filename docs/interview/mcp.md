...
我在 ZCLI 中实现了一个真实的 stdio MCP 工具桥接层：Agent 先通过 `connect_mcp` 启动并连接外部 Server，完成 MCP 初始化和工具发现，再把远端工具以 `mcp__server__tool` 的形式动态合入下一轮 LLM 工具池。

这套设计参考了 `learn-claude-code` s19/s20 的后绑定工具思路，但没有停留在教学版 mock，而是实现了真实子进程、JSON-RPC 消息、超时、权限审批和进程回收。
...
它描述怎样启动一个 stdio Server：命令、参数、环境变量、工作目录和请求超时。
...
MCP 配置中的 `command` 本质上是本地可执行命令，所以调用 `connect_mcp` 必须得到用户批准：
...
## 20. 如何测试真实 MCP，而不是只测 mock

测试中启动了一个真实 Python 子进程作为 MCP Server，通过 stdin/stdout 处理 JSON-RPC。
...
## 21. 为什么没有直接依赖 MCP SDK

这一版没有引入 MCP SDK，而是实现了最小 stdio Tools 子集，原因是：

- 可以把 initialize、list、call 和 JSON-RPC 关联机制讲清楚；
- 避免为只使用 Tools 引入更大的运行时依赖；
- 更容易控制错误包装、权限和 ZCLI 工具池集成。

代价也很明确：协议面较窄，随着 HTTP、OAuth、Resources 和通知加入，自研协议层的维护成本会快速上升。生产化下一阶段更适合迁移到官方 SDK，并保留现有 `MCPManager` 作为适配层。

## 22. 当前限制

目前实现的是“真实 MCP 工具桥接”，不是完整 MCP Runtime：

1. 只支持 stdio，不支持 HTTP、SSE、WebSocket；
2. 不支持 OAuth 和令牌刷新；
3. 不支持 Resources、Prompts；
4. 不处理 Server 发起的请求和反向通知；
5. 不订阅工具列表变化；
6. 断线后不会自动重连；
7. 同一 Server 的请求由锁串行执行；
8. annotations 当前主要处理 read-only 和 destructive 提示。
...
1. 使用官方 SDK 替换手写协议层；
2. 增加 Streamable HTTP transport；
3. 增加断线状态、指数退避和自动重连；
...
> 我在 ZCLI 中实现了真实的 stdio MCP 工具桥接。核心思路是后绑定工具：Agent 启动时只提供 connect_mcp，用户批准连接后，客户端启动配置中的子进程，通过 JSON-RPC 完成 initialize、initialized 和 tools/list，再把远端工具转换为 mcp__server__tool 命名并合入动态工具池。ToolRegistry 的 definitions 每次模型调用前都会重新计算，所以连接后的下一轮模型马上能看到新工具。调用时 Manager 根据公共名称找到对应 Client 和远端原名，发送 tools/call，结果再进入原有 Hook、权限、上下文压缩和 Session 链路。安全上，连接本地进程和 destructive 工具都要审批，cwd 不能逃逸工作区；退出时按 EOF、terminate、kill 分级回收子进程。目前完成的是 stdio Tools 核心链路，不宣称支持 OAuth、Resources 或完整远程 transport。

## 35. 两分钟讲解

> 这个功能解决的是 Agent 外部能力扩展问题。以前接 Jira、知识库或部署平台都要在 Agent 里手写工具，MCP 则把发现和调用标准化。
>
> 实现分为三层。第一层是配置层，按用户级、项目级和项目本地三级合并 mcp.json，校验 command、args、env、cwd 和名称冲突，密钥可以通过环境变量引用。第二层是协议层，StdioMCPClient 用 subprocess 启动 Server，通过 stdin/stdout 发送逐行 JSON-RPC，连接时执行 initialize、notifications/initialized 和 tools/list，调用时执行 tools/call。stdout 和 stderr 分别由后台线程消费，响应进入 Queue，主线程可以设置超时；同一 Client 用锁串行请求，先保证协议状态正确。第三层是 Agent 集成层，MCPManager 把远端工具包装成 MCPTool，并用 mcp__server__tool 解决重名。ToolRegistry 每次 LLM 请求都动态拼接 MCP definitions，所以 connect_mcp 完成后的下一轮请求就会出现新工具。
>
...
>
> 测试不是只 mock 一个函数，而是真的启动本地 MCP 子进程，验证完整握手、发现、动态工具刷新、调用、权限和关闭。当前边界是只支持 stdio Tools；如果继续生产化，我会用官方 SDK 扩展 Streamable HTTP、OAuth、通知、自动重连和并发请求。
